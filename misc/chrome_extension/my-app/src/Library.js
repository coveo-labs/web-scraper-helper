...
class Library extends Component {
  ...
  render() {
    ...
    let savedSpecs = Object.keys(this.state.specs).map(name=>
      <option key={name} value={name}>{name}</option>
    );

    let saveDisabled = (currentSpec==='__empty');

    console.log('RENDEAR: ', currentSpec);
    return (
      <div id="setup">
  ...
  loadSpec(e) {
    ...
    if (name === '__create') {
      ...
      let specs = this.state.specs;
      specs[name] = JSON.stringify( [{"for":{"urls":[".*"]},"exclude":[],"metadata":{}}] );

      state.specs = specs;
    }

    this.setState(state, ()=>{
      let specs = this.state.specs[name];
      if (specs && this.props.onLoad) {
        Storage.set(specs);
      }
    });
  }
